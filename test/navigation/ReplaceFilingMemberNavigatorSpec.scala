...
class ReplaceFilingMemberNavigatorSpec extends SpecBase {
  ...
  "Replace Filing Member Navigator" when {
    ...
    "in Normal mode" must {
      ...
      "if user select no  on contact by telephone page then go to rfm contact address page " in {
        navigator.nextPage(
          RfmContactByTelephonePage,
          NormalMode,
          emptyUserAnswers.setOrException(RfmContactByTelephonePage, false)
        ) mustBe controllers.rfm.routes.RfmAddSecondaryContactController.onPageLoad(NormalMode)
      }
      ...
      "go to RfmContactAddress page from Rfm Secondary Telephone Controller" in {
        navigator.nextPage(
      ...
      "go to RfmContactAddress Controller if they choose not to nominate a secondary contact number" in {
        navigator.nextPage(
          RfmSecondaryPhonePreferencePage,
          NormalMode,
