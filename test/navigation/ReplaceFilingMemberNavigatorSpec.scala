...
class ReplaceFilingMemberNavigatorSpec extends SpecBase {
  ...
  "Replace Filing Member Navigator" when {
    ...
    "in Normal mode" must {
      ...
      "must go from a page that doesn't exist in the route map to RFM journey recovery" in {
        case object UnknownPage extends Page
        navigator.nextPage(UnknownPage, NormalMode, UserAnswers("id")) mustBe controllers.rfm.routes.RfmJourneyRecoveryController.onPageLoad
      }
    ...
    "in Check mode" must {
      ...
      "must go from a page that doesn't exist in the route map to RFM journey recovery" in {
        case object UnknownPage extends Page
        navigator.nextPage(UnknownPage, CheckMode, UserAnswers("id")) mustBe controllers.rfm.routes.RfmJourneyRecoveryController.onPageLoad
      }
