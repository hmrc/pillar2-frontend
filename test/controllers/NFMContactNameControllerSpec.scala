...
class NFMContactNameControllerSpec extends SpecBase {
  ...
  "NFMContactName Controller" when {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, routes.NFMContactNameController.onPageLoad(NormalMode).url)
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, routes.NFMContactNameController.onPageLoad(NormalMode).url)
