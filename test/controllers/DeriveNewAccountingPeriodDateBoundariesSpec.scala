...
class DeriveNewAccountingPeriodDateBoundariesSpec extends AnyFreeSpec with Matchers {
  ...
  "deriveNewAccountingPeriodDateBoundaries" - {
    ...
    "returns the end date as start boundary when canAmendEndDate is false and is closer than canAmendStartDate" in {
      val periods = Seq(
        period(2025),
        period(2024),
        period(2023, canAmendStart = false, canAmendEnd = false)
      )
