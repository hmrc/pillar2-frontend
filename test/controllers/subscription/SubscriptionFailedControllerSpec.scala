...
class SubscriptionFailedControllerSpec extends SpecBase {
  ...
  "Subscription Failed Controller" when {
    ...
    "must return OK and the continue view" in {
      ...
      running(application) {
        val request = FakeRequest(GET, controllers.subscription.routes.SubscriptionFailedController.onPageLoad.url)

        val result = route(application, request).value
        val view   = application.injector.instanceOf[RegistrationOrSubscriptionFailedView]
