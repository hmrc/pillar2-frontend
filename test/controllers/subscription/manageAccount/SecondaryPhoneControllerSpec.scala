...
class SecondaryPhoneControllerSpec extends SpecBase {
  ...
  "SecondaryPhone Controller for Organisation View Contact details" when {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(formProvider.fill("1234567"), "name", isAgent = false, Some("ABC Intl"), Some("somePillar2Ref"))(
          request,
          applicationConfig,
          messages(application)
        ).toString
  ...
  "SecondaryPhone Controller for Agent View Contact details" when {
