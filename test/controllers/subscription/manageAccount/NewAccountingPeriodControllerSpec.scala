...
class NewAccountingPeriodControllerSpec extends SpecBase with Generators {
  ...
  val chosenAccountingPeriod: ChosenAccountingPeriod =
    ChosenAccountingPeriod(
      selectedAccountingPeriod = amendablePeriod.toAccountingPeriod,
      startDateBoundary = None,
      endDateBoundary = None
    )

  private val localDataWithAmendablePeriods: SubscriptionLocalData =
    emptySubscriptionLocalData.copy(
      subAccountingPeriod = Some(amendablePeriod.toAccountingPeriod),
      accountingPeriods = Some(Seq(amendablePeriod))
    )

  "NewAccountingPeriodController" must {
    ...
    "redirect to Journey Recovery" when {
      ...
      "no user selected accounting period present" in {
        ...
        val application = applicationBuilder(subscriptionLocalData = Some(subscriptionDataNoSelectedPeriod)).build()
    ...
    "return OK and the correct view for a GET when a previous answer exists" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(
          formProvider(chosenAccountingPeriod).fill(accountingPeriod),
          chosenAccountingPeriod,
          isAgent = false,
          organisationName = None,
          plrReference = testPillar2Id,
    ...
    "must save and redirect to the check your answers page when a valid answer is submitted" in {
      ...
      running(application) {
        ...
        val request = FakeRequest(POST, routes.NewAccountingPeriodController.onSubmit(NormalMode).url)
        ...
        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual routes.AmendAccountingPeriodCYAController.onPageLoad().url
      }
    }
