  ...
  "SecondaryTelephonePreference Controller for View Contact details" when {
    ...
    "must return OK and the correct view for a GET if no previous data is found" in {
      ...
      running(application) {
        ...
        contentAsString(result) mustEqual view(formProvider.fill(false), CheckMode, "name", false)(
          request,
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(formProvider.fill(true), CheckMode, "name", false)(
          request,
          appConfig(application),
          messages(application)
        ).toString
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      running(application) {
        ...
        contentAsString(result) mustEqual view(boundForm, CheckMode, "name", false)(request, appConfig(application), messages(application)).toString
      }
    }
    ...
    "must update subscription data and redirect to the next page when the user answers no" in {
      ...
      running(application) {
        ...
        verify(mockNavigator).nextPage(SubSecondaryPhonePreferencePage, CheckMode, expectedUserAnswers)
      }
    }
