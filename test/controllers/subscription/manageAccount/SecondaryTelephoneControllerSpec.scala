  ...
  "SecondaryTelephone Controller for View Contact details" when {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(formProvider.fill("1234567"), NormalMode, "name", false)(
          request,
          appConfig(application),
          messages(application)
        ).toString
    ...
    "must update subscription data and redirect to the next page" in {
      ...
      running(application) {
        ...
        verify(mockNavigator).nextPage(SubSecondaryCapturePhonePage, CheckMode, expectedUserAnswers)
      }
    }
