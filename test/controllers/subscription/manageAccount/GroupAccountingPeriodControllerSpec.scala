  ...
  "GroupAccountingPeriod Controller for View Contact details" when {
    ...
    "must return OK and the correct view for a GET if no previous data is found" in {
      ...
      running(application) {
        ...
        contentAsString(result) mustEqual view(formProvider(true).fill(emptySubscriptionLocalData.subAccountingPeriod), CheckMode, false)(
          request,
    ...
    "must return OK and the correct view for a GET if page has previously been answered" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(formProvider(true).fill(date), CheckMode, false)(
          request,
          appConfig(application),
          messages(application)
        ).toString
