...
class NonUKBankControllerSpec extends SpecBase {
  ...
  "NonUKBank Controller" when {
    ...
    "must redirect existing contact details when valid data is submitted" in {
      ...
      running(application) {
        ...
        val result = route(application, request).value
        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual controllers.repayments.routes.ExistingContactDetailsController.onPageLoad(clientPillar2Id = None).url
      }
    }
