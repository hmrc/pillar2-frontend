...
class RepaymentsPhoneDetailsControllerSpec extends SpecBase {
  ...
  "Repayments Phone Details Controller" when {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        ...
        val request =
          FakeRequest(GET, controllers.repayments.routes.RepaymentsPhoneDetailsController.onPageLoad(NormalMode).url)
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      val ua = emptyUserAnswers
        ...
        .success
        .value
        .set(RepaymentsPhoneDetailsPage, "12345")
        .success
        .value
    ...
    "must redirect to Under Construction Page when valid data is submitted" in {
      ...
      running(application) {
        ...
        val request =
          FakeRequest(POST, controllers.repayments.routes.RepaymentsPhoneDetailsController.onSubmit(NormalMode).url)
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      running(application) {
        ...
        val boundForm = formProvider("ABC Limited").bind(Map("phoneNumber" -> "abc"))
        val view      = application.injector.instanceOf[RepaymentsPhoneDetailsView]
        val result    = route(application, request).value
