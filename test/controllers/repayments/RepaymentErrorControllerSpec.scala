...
class RepaymentErrorControllerSpec extends SpecBase {
  ...
  "Partial Account Name Match" must {
    "must redirect to error page if the feature flag is false" in {
      ...
      running(application) {
        val request = FakeRequest(GET, routes.RepaymentErrorController.onPageLoadPartialNameError().url)
    ...
    "must return the correct view if the feature flag is true" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(formProvider(), None, "James")(request, appConfig(application), messages(application)).toString
      }
    }
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, routes.RepaymentErrorController.onPageLoadPartialNameError().url)
        ...
        contentAsString(result) mustEqual view(boundForm, None, "James")(request, appConfig(application), messages(application)).toString
      }
    }
    ...
    "must redirect to input name page if valid selection is submitted" in {
      ...
      running(application) {
        val request = FakeRequest(POST, routes.RepaymentErrorController.onSubmitPartialNameError().url)
