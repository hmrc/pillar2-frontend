...
class RegistrationInProgressControllerSpec extends SpecBase {
  ...
  "RegistrationInProgress Controller" when {
    ...
    "return OK and the correct view for a GET with PLR reference" in {
      ...
      running(application) {
        ...
        val view = application.injector.instanceOf[RegistrationInProgressView]

        status(result) mustEqual OK
        contentAsString(result) mustEqual view(plrReference)(request, messages(application), applicationConfig).toString
      }
    }
