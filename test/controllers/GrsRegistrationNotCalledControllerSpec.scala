...
class GrsRegistrationNotCalledControllerSpec extends SpecBase {
  ...
  "GrsRegistrationNotCalled  Controller" when {
    ...
    "must return OK and the correct view for a GET for UPE" in {
      ...
      running(application) {
        val request = FakeRequest(GET, controllers.routes.GrsRegistrationNotCalledController.onPageLoadUpe.url)
    ...
    "must return OK and the correct view for a GET for NFM" in {
      ...
      running(application) {
        val request = FakeRequest(GET, controllers.routes.GrsRegistrationNotCalledController.onPageLoadNfm.url)
