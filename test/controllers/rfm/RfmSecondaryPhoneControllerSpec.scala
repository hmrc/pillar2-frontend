...
class RfmSecondaryPhoneControllerSpec extends SpecBase {
  ...
  "RFM SecondaryPhone Controller" when {
    ...
    "must return OK and the correct view for a GET if no previous data is found" in {
      ...
      running(application) {
        ...
        val view = application.injector.instanceOf[RfmSecondaryPhoneView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val ua = emptyUserAnswers
        .setOrException(RfmSecondaryContactNamePage, "name")
        .setOrException(RfmSecondaryPhonePreferencePage, true)
        .setOrException(RfmSecondaryCapturePhonePage, "1234567")
    ...
    "must return a Bad Request and errors when invalid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, controllers.rfm.routes.RfmSecondaryPhoneController.onSubmit(NormalMode).url)
