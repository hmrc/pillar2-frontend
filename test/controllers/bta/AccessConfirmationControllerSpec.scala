...
import views.html.bta.AccessConfirmationView

class AccessConfirmationControllerSpec extends SpecBase {

  "AccessConfirmation Controller" must {
    ...
    "must return OK and the correct view for a GET - bta feature true" in {
      ...
      running(application) {
        val request = FakeRequest(GET, controllers.bta.routes.AccessConfirmationController.onPageLoad.url)

        val result = route(application, request).value

        val view = application.injector.instanceOf[AccessConfirmationView]
