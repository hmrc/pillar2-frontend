...
class UnauthorisedAgentAffinityControllerSpec extends SpecBase {
  ...
  "UnauthorisedAgentAffinityController" should {
    ...
    "return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, controllers.routes.UnauthorisedAgentAffinityController.onPageLoad.url)

        val result = route(application, request).value

        val view = application.injector.instanceOf[UnauthorisedAgentView]
