  ...
  "Enter And Submit Client Pillar 2 Id" must {
    ...
    "must return the correct view if the feature flag is true and user is agent" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(formProvider())(request, appConfig(), messages(application)).toString
      }
    }
  ...
  "Confirm Client Details" must {
    ...
    "return the correct view if client pillar 2 id and organisation name is in user answers" in {
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view("Some Org", "XMPLR0123456789")(request, appConfig(), messages(application)).toString
      }
    }
