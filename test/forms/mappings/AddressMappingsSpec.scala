...
class AddressMappingsSpec extends SpecBase with AddressMappings {
  ...
  "Transformation utility methods" should {
  ...
  "xssFirstOptionalPostcode" should {
  ...
  "xssFirstMandatoryPostcode" should {
    ...
    "handle postcode case conversion" in {
    ...
    "return required error for missing postcode when country is not GB" in {
      val data   = Map("postcode" -> "", "countryCode" -> "US")
      val result = form.bind(data)
      result.errors.head.message mustBe "address.postcode.error.required"
    }

    "return required error for missing postcode and country" in {
    ...
    "handle various UK postcode formats" in {
      val testCases = Seq(
        ("M11AA", "M1 1AA"),
        ("M601AA", "M60 1AA"),
        ("CR06XH", "CR0 6XH"),
    ...
    "handle XSS characters with special regex characters" in {
      val testCases = Seq(
        "SW1A&1AA",
        "SW1A>1AA",
        "SW1A<1AA",
    ...
    "handle UK postcodes with single letter area" in {
      val data   = Map("postcode" -> "M1 1AA", "countryCode" -> "GB")
      val result = form.bind(data)
      result.value mustBe Some(("M1 1AA", "GB"))
    }

    "handle mixed case normalisation" in {
      val data   = Map("postcode" -> "sw1a1aA", "countryCode" -> "GB")
      val result = form.bind(data)
      result.value mustBe Some(("SW1A 1AA", "GB"))
    }
    ...
    "handle very short postcodes for non-GB countries" in {
      val data   = Map("postcode" -> "1", "countryCode" -> "US")
      val result = form.bind(data)
      result.value mustBe Some(("1", "US"))
    }

    "handle postcode exactly at maxPostCodeLength for non-GB" in {
