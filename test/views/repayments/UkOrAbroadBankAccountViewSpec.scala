...
class UkOrAbroadBankAccountViewSpec extends ViewSpecBase {
  ...
  "UK or Abroad Bank Account View" when {
    ...
    "page loaded" should {
      ...
      "have a title" in {
        view.title() mustBe "What type of account will the refund be sent to? - Report Pillar 2 Top-up Taxes - GOV.UK"
      }

      "have a unique H1 heading" in {
        val h1Elements: Elements = view.getElementsByTag("h1")
        h1Elements.size() mustBe 1
        h1Elements.text() mustBe "What type of account will the refund be sent to?"
      }
