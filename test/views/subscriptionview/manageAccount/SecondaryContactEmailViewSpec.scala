  ...
  "SecondaryContactEmailView" should {
    ...
    "show appropriate error when the email field is left empty" in {
      val errorView = Jsoup.parse(
        page(form.bind(Map("emailAddress" -> "")), isAgent = false, secondaryContactName = contactName, organisationName = None)(
          request,
          appConfig,
          messages
        ).toString()
    ...
    "show error when email format is invalid" in {
      val errorView = Jsoup.parse(
        page(form.bind(Map("emailAddress" -> "invalid-email")), isAgent = false, secondaryContactName = contactName, organisationName = None)(
          request,
          appConfig,
          messages
        ).toString()
    ...
    "show error when email is too long (over 132 characters)" in {
      ...
      val errorView = Jsoup.parse(
        page(form.bind(Map("emailAddress" -> longEmail)), isAgent = false, secondaryContactName = contactName, organisationName = None)(
          request,
          appConfig,
          messages
        ).toString()
