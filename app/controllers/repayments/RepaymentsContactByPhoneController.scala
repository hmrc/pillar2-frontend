...
class RepaymentsContactByPhoneController @Inject() (
    ...
    (identify andThen getSessionData andThen requireSessionData andThen journeyGuard) { request =>
      ...
      request.userAnswers
        ...
        .map { contactName =>
          ...
          val preparedForm = request.userAnswers.get(RepaymentsContactByPhonePage) match {
            case None        => form
          ...
          formProvider(name)
            ...
            .fold(
              ...
              value =>
                ...
                } yield Redirect(navigator.nextPage(RepaymentsContactByPhonePage, mode, updatedAnswers))
            )
        }
