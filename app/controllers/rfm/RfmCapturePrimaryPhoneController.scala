...
import pages.{RfmCapturePrimaryPhonePage, RfmPrimaryContactNamePage}
...
class RfmCapturePrimaryPhoneController @Inject() (
  ...
  view:                             RfmCapturePrimaryPhoneView,
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData) { implicit request =>
    request.userAnswers
      ...
      .map { contactName =>
        ...
        val preparedForm = request.userAnswers.get(RfmCapturePrimaryPhonePage).map(form.fill).getOrElse(form)
      ...
      .map { contactName =>
        formProvider(contactName)
          ...
          .fold(
            ...
            value =>
              for {
                updatedAnswers <- Future.fromTry(request.userAnswers.set(RfmCapturePrimaryPhonePage, value))
                _              <- userAnswersConnectors.save(updatedAnswers.id, Json.toJson(updatedAnswers.data))
              } yield Redirect(navigator.nextPage(RfmCapturePrimaryPhonePage, mode, updatedAnswers))
          )
      }
