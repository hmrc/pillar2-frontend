...
class RfmContactByPhoneController @Inject() (
  val userAnswersConnectors:        UserAnswersConnectors,
  @Named("RfmIdentifier") identify: IdentifierAction,
  getData:                          DataRetrievalAction,
  requireData:                      DataRequiredAction,
  journeyGuard:                     RfmDataJourneyGuardAction,
  formProvider:                     RfmContactByPhoneFormProvider,
      ...
      .map { contactName =>
        ...
        form
          ...
          .fold(
            ...
            value =>
              for {
                updatedAnswers <- Future.fromTry(request.userAnswers.set(RfmContactByPhonePage, value))
                _              <- userAnswersConnectors.save(updatedAnswers.id, Json.toJson(updatedAnswers.data))
              } yield Redirect(navigator.nextPage(RfmContactByPhonePage, mode, updatedAnswers))
          )
      }
