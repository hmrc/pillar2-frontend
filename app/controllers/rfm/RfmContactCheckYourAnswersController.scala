...
import models.{InternalIssueError, ReadSubscriptionError, UnexpectedResponse}
...
class RfmContactCheckYourAnswersController @Inject() (
  ...
  def onPageLoad: Action[AnyContent] = (featureAction.rfmAccessAction andThen identity andThen getData andThen requireData).async {
    implicit request =>
  ...
  private def replaceFilingMemberDetails(request: DataRequest[AnyContent])(implicit hc: HeaderCarrier): Future[Result] =
    (for {
      ...
      .recover {
        case InternalIssueError | UnexpectedResponse | ReadSubscriptionError =>
