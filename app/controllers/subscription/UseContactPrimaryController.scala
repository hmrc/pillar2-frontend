...
class UseContactPrimaryController @Inject() (
  ...
  private def fmNoID(mode: Mode)(using request: DataRequest[AnyContent]) =
    (for {
      ...
      val contactTel = if telPref then request.userAnswers.get(FmCapturePhonePage) else None
      request.userAnswers.get(SubUsePrimaryContactPage) match {
        case Some(value) => Ok(view(form.fill(value), mode, contactSummaryList(contactName, contactEmail, contactTel)))
        case None        => Ok(view(form, mode, contactSummaryList(contactName, contactEmail, contactTel)))
      }
    }).getOrElse(Redirect(controllers.routes.JourneyRecoveryController.onPageLoad()))
  ...
  private def upeNoID(mode: Mode)(using request: DataRequest[AnyContent]) =
    (for {
      ...
      val contactTel = if telPref then request.userAnswers.get(UpeCapturePhonePage) else None
      request.userAnswers.get(SubUsePrimaryContactPage) match {
        case Some(value) => Ok(view(form.fill(value), mode, contactSummaryList(contactName, contactEmail, contactTel)))
        case None        => Ok(view(form, mode, contactSummaryList(contactName, contactEmail, contactTel)))
      }
  ...
  private[controllers] def contactSummaryList(contactName: String, contactEmail: String, contactTel: Option[String])(using
    ...
    SummaryListViewModel(
      rows = Seq(
        ...
        contactTel.map(tel => SummaryListRowViewModel(key = "useContactPrimary.phone", value = ValueViewModel(HtmlFormat.escape(tel).toString)))
      ).flatten
    )
}
