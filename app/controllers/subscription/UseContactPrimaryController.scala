...
class UseContactPrimaryController @Inject()(
        ...
        form
          ...
          .fold(
            formWithErrors => Future.successful(
              BadRequest(view(formWithErrors, mode, contactDetail.contactName, contactDetail.ContactEmail, contactDetail.ContactTel))),
            value =>
              value match {
                case true =>
                  for {
                    ...
                    updatedAnswers3 <- Future.fromTry(contactDetail.ContactTel.map(
                      updatedAnswers2.set(subPrimaryCapturePhonePage, _)).getOrElse(Success(updatedAnswers2)))
  ...
  private def contactDetail(request: DataRequest[AnyContent]): Either[Result, SubscriptionContactDetails] = {
    request.userAnswers.get(NominateFilingMemberPage).flatMap { registered =>
      if (registered) {
        request.userAnswers.get(fmRegisteredInUKPage).map { ukBased =>
          if (!ukBased) {
            (for {
              ...
              Right(SubscriptionContactDetails(contactName, contactEmail, request.userAnswers.get(fmCapturePhonePage)))
