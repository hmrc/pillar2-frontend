...
import pages.{GrsFilingMemberStatusPage, GrsUpStatusPage, fmCapturePhonePage, fmContactEmailPage, fmContactNamePage, fmEntityTypePage, fmGRSResponsePage, fmNameRegistrationPage, fmPhonePreferencePage, fmRegisteredAddressPage, fmRegisteredInUKPage, subAccountingPeriodPage, subMneOrDomesticPage, subPrimaryContactNamePage, subRegisteredAddressPage, upeCapturePhonePage, upeContactEmailPage, upeContactNamePage, upeEntityTypePage, upeGRSResponsePage, upeNameRegistrationPage, upePhonePreferencePage, upeRegisteredAddressPage, upeRegisteredInUKPage}
...
trait SubscriptionHelpers {
  ...
  def getUpeStatus(request: DataRequest[AnyContent]): RowStatus = {
    request.userAnswers.get(upeRegisteredInUKPage).map { ukBased =>
      if (!ukBased) {
        (for {
