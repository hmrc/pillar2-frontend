    ...
    answers.get(RepaymentsContactByPhonePage).map { answer =>
      ...
      SummaryListRowViewModel(
        ...
        actions = Seq(
          ActionItemViewModel(
            "site.change",
            controllers.repayments.routes.RepaymentsContactByPhoneController.onPageLoad(CheckMode).url
          )
            .withVisuallyHiddenText(messages("repaymentsContactByPhone.change.hidden"))
