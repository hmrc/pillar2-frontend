...
object RfmRegisteredAddressSummary {
  def row(answers: UserAnswers, countryOptions: CountryOptions)(implicit messages: Messages): Option[SummaryListRow] =
    answers.get(rfmNfmRegisteredAddressPage).map { answer =>
      val country = countryOptions.getCountryNameFromCode(answer.countryCode)
      SummaryListRowViewModel(
        key = "rfm.nfmRegisteredAddress.checkYourAnswersLabel",
        value = ValueViewModel(HtmlContent(answer.fullAddress ++ country)),
        actions = Seq(
          ActionItemViewModel("site.change", controllers.rfm.routes.NfmRegisteredAddressController.onPageLoad().url)
            .withVisuallyHiddenText(messages("rfm.nfmRegisteredAddress.change.hidden"))
        )
      )
