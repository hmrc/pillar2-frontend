    ...
    http
      ...
      .flatMap {
        ...
        case unprocessableEntityResponse if unprocessableEntityResponse.status.equals(UNPROCESSABLE_ENTITY) =>
          val responseBody = unprocessableEntityResponse.body
          if (responseBody.contains("subscription is being processed") || responseBody.contains("please retry")) {
            Future.failed(SubscriptionProcessingError)
          } else {
