...
case class TolerantAddress(
  ...
  //    case (Some(line1), Some(line2), Some(country)) => Some(Address(line1, line2, addressLine3, addressLine4,townOrCity,region, postcode, country))
  //    case (_, _, None)                              => None
  ...
  object TolerantAddress {
    ...
    //  }
    //
    //  val postCodeLookupReads: Reads[Seq[TolerantAddress]] = Reads { json =>
    //    json
    //      .validate[Seq[JsValue]]
    //      .flatMap { addresses =>
    //        addresses.foldLeft[JsResult[List[TolerantAddress]]](JsSuccess(List.empty)) { (addresses, currentAddress) =>
    //          for {
